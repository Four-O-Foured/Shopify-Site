  ...
  return (
    <div className="mt-20">
      <form onSubmit={handleSubmit(formSubmit)}>
        <input
          className="text-4xl w-1/2 p-3 block text-[#37353E] border-b-2 border-[#e6e5e5]  font-semibold outline-none focus:border-b-2 focus:border-[#37353E] "
          {...register("username", { required: "Username is required" })}
          type="text"
          placeholder="Username"
        />
        <p className="text-red-400 my-1">{errors.username?.message}</p>
        <input
        ...
        <div className="flex mt-18 gap-8 items-center">
          ...
          <div className="flex gap-1">
            ...
            <Link to="/login" className="text-md mt-1 underline text-cyan-600">
              <h1>Login</h1>
            </Link>
          </div>
        </div>
